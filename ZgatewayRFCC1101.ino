...
#define MAXSENDBUFFERLENGTH 700
...
void setupRFCC1101() {
  radio.setup();
  enterReceiveMode();
  trc(F("ZgatewayCC1101 setup done "));
}
...
boolean RFCC1101toMQTT() {
  ...
  if (radio.isBufferReady()) {
    ...
    trc("CC1101 received 433 blocks");
    String msg = "";
...
void MQTTtoRFCC1101(char * topicOri, char * datacallback) {
  ...
  if (topic.equals(subjectMQTTtoRFCC1101)) {
    trc("received pattern for cc1101");
    ...
    for (int i = 0; i < msg.length(); i++) {
      ...
      if (msg[i] == '+') {
        trc("wait for next part of timing pattern");
        return;
      }
